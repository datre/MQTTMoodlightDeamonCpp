...
Serial::Serial(string serialport, int baud)
{
    fd = serialport_init(serialport.c_str(), baud);
}
...
int Serial::serialport_write(const char* str, int length){
    int len = length;
#ifdef _WIN32
    DWORD n;
    DWORD errors;
...
int Serial::serialport_read_until(char* buf, char until, int timeout){
    char b[2] = " ";
    int i=0;
    do {
#ifdef _WIN32
...
//            usleep( 10 ); // wait 10 msec try again
    ...
    } while( b[0] != until and i<timeout);

    buf[i] = '\0';  // null terminate the string
    return 0;
}

int Serial::serialport_readbyte(char *b){
#ifdef _WIN32
    DWORD n;
...
SERIALPORT_HANDLE Serial::serialport_init(const char *serialport, int baud){
...
/*
int Serial::serialport_restoresettings(SERIALPORT_TERMIOS *oldsettings){
